...
class ImguiDInputDeviceDS3 : IDirectInputDevice8A {
public:
    GUID deviceGuid;
    IDirectInput8A* dinput;
    IDirectInputDevice8A* device;
    LPCDIDATAFORMAT dataFormat;
    HWND cooperativeLevelWnd;
    DWORD cooperativeLevelWord;
    ImguiDInputDeviceDS3() : ImguiDInputDeviceDS3({}, nullptr, nullptr) {}
    ImguiDInputDeviceDS3(GUID deviceGuid, IDirectInput8A* dinput, IDirectInputDevice8A* device) : deviceGuid(deviceGuid), dinput(dinput), device(device) {
        dataFormat = {};
        cooperativeLevelWnd = {};
        cooperativeLevelWord = {};
        if (device != nullptr) {
            device->AddRef();
        }
    }
    virtual ~ImguiDInputDeviceDS3() {
        if (device != nullptr) {
            device->Release();
        }
    }
